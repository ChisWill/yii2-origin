$(function () {
    ...
    $("#settingContent").on('click', '.updateSetting', function () {
        var $this = $(this);
        if ($("#changeEditBtn").attr('mode') == 2 && $this.children('input').length === 0) {
            $this.data('oldValue', $this.html());
            if ($this.data('field') === 'comment') {
                var style = {'width': '90%'};
            } else {
                var style = {'width': '70%', 'height': '10px', 'padding': '1px'};
            }
            $this.html($('<input>').val($this.html()).addClass('updateSettingInput').css(style)).children().focus();
        }
    });
    $("#settingContent").on('blur', '.updateSettingInput', function () {
        var $input = $(this),
            $this = $input.parent();
        if ($this.data('oldValue') == $input.val()) {
            $this.html($this.data('oldValue'));
        } else {
            $.post($("#updateSettingUrl").val(), {id: $this.parents('tr').data('id'), field: $this.data('field'), value: $input.val()}, function (msg) {
                if (msg.state) {
                    $this.html($input.val());
                } else {
                    $.alert(msg.info);
