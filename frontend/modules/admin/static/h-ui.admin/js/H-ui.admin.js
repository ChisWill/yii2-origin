...
function Hui_admin_tab(obj){
	if($(obj).attr('_href')){
		...
		if(!bStop){
			creatIframe(_href,_titleName, $(obj));
			min_titleList();
		}
		else{
...
function creatIframe(href,titleName, $navLink){
	var topWindow=$(window.parent.document);
	var show_nav=topWindow.find('#min_title_list');
	...
	iframeBox.hide();
	iframe_box.append('<div class="show_iframe"><div class="loading"></div><iframe frameborder="0" src='+href+' data-maintitle="' + $navLink.parents("dd").prev().find("span").html() + '" data-subtitle="' + titleName + '"></iframe></div>');
