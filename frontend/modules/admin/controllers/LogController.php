 ...
 * @author ChisWill
 */
class LogController extends \admin\components\Controller
{
    /**
     * @authname 运行日志
     */
    public function actionLogList()
    {
        ...
        return $this->render('logList', compact('html', 'fileHtml'));
    }
    ...
    public function actionActionList()
    {
        ...
        $html = $query->getTable([
            ...
            'action' => ['header' => '操作描述', 'width' => '80px', 'value' => function ($row) use ($permissionData) {
                ...
                if (in_array($action, ['ajax-update', 'toggle', 'login', 'password', 'logout'])) {
                    switch ($action) {
                        case 'password':
                            return '修改密码';
                        case 'login':
    ...
    public function actionSqlList()
    {
        ...
        return $this->render('sqlList', compact('html'));
    }
