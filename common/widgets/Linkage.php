...
class Linkage extends \yii\base\Widget
{
    ...
    protected function initColumns()
    {
        ...
        foreach ($columns as $index => $value) {
            // 略过未配置的项
            if ($index === '' || isset($value['field']) && $value['field'] === '') {
                unset($columns[$index]);
                continue;
            }
    ...
    protected function renderUl($pids = [], $recursionLevel = 0)
    {
        ...
        $this->ulOptions['data'] = ['level' => $recursionLevel];
        ...
        foreach ($this->columns as $options) {
            ...
            if (($type = ArrayHelper::getValue($options, 'type')) !== null) {
                ...
                switch ($type) {
                    ...
                    case 'text':
                        ...
                        break;
                    case 'password':
                        $colOptions['data-action'] = 'passwordUpdate';
                        $content = '***';
                        break;
                    case 'toggle':
            ...
            if ($index === 0) {
                ...
                if ($type && $type !== 'checkbox') {
                    throw new InvalidParamException('第一列的 [type] 属性只能设置成 `checkbox` 或不设置！');
                }
                // 如果是顶级元素，则不需要前导图片
    ...
    public static function deleteItem()
    {
        ...
        if (method_exists($model, $deleteMethod)) {
        ...
        } elseif (in_array('state', $model->attributes())) {
            ...
            return $model::updateAll($updateMap, [$keyField => $ids]);
        } else {
            return $model::deleteAll([$keyField => $ids]);
        }
    }
