...
class Table extends \yii\base\Widget
{
    ...
    /**
     * @var boolean|string|array 是否导出Excel
     * Excel文件名优先取该配置的字符串值
     * 未设置时取当前页面的title值
     * 类型为array时，格式：
     * [
     *     'header' => 'Title',
     *     'columns' => [1, 2, 4],// 表示只选取配置列中第x列的数据
     * ]
     */
    public $export = false;
    /**
    ...
    protected function initColumns()
    {
        ...
        foreach ($columns as $index => $value) {
            // 略过未配置的项
            if (!$this->isOperationColumn($value) && ($index === '' || isset($value['field']) && $value['field'] === '')) {
                unset($columns[$index]);
                continue;
            }
    ...
    protected function exportExcel()
    {
        ...
        if (is_string($this->export)) {
            $title = $this->export;
        } elseif (is_array($this->export)) {
            $title = ArrayHelper::getValue($this->export, 'header', Yii::$app->view->title);
            $columns = ArrayHelper::getValue($this->export, 'columns', []);
            $i = 1;
            foreach ($this->columns as $key => $value) {
     ...
     * @param  array $primaryKey  $model->primaryKey() 的返回结果
     * @return string|array       主键的值
     */
    protected function getKeyValue($value, $primaryKey = [])
    {
        $key = [];
        !$primaryKey && $primaryKey = $this->key;
        array_walk($primaryKey, function ($keyField) use (&$key, $value) {
    ...
    protected function getContent($value, $key, $options)
    {
        ...
        if (!empty($options['value'])) {
            if (is_string($options['value'])) {
                if (is_object($value)) {
                    $content = call_user_func([$value, $options['value']], $key, $options['field']);
                } else {
                    throw new InvalidParamException('查询的结果集必须是对象数组才能将配置项 [value] 设置成字符串类型！');
                }
