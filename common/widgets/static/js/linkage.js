$(function () {
    ...
    $(container).coffee({
        click: {
            ...
            // 密码输入框型的点击修改事件
            '[data-action="passwordUpdate"]': function () {
                var $this = $(this);
                if ($this.find('input').length === 0) {
                    var $input = $('<input>').attr('type', 'password').data('oldValue', $this.html()).keyup(function (event) {
                        var key = $.getEventKey(event);
                        if (key === $.keyCode['ENTER']) {
                            $(this).trigger('blur');
                        }
            ...
            '.linkage-add-link': function () {
                ...
                $row.find('[data-field]').each(function (index) {
                    var style = $(this).attr('style') || $(this).parent().attr('style'),
                        child = $("<input>").attr('name', 'Linkage[' + $(this).data('field') + ']');
                    if ($(this).data('action') === 'textUpdate') {
                        child.attr({
                            'type': 'text',
                            'placeholder': $(".linkage-header").children().eq(index).html()
                        });
                    } else {
                        child.attr('type', 'hidden');
                    }
                    $form.append($("<" + colTag + ">").attr('style', style).append(child));
                });
                var $addLink = $("<a>").addClass('addItemLink').data({'pid': $this.data('pid'), 'addchild': $(this).data('addchild')}).html('添加'),
                        ...
                        if (msg.state) {
                                ...
                                if (index === 0) {
                                    ...
                                    $(this).find('.linkage-add-link').each(function () {
                                        ...
                                        if ($(this).data('pid') == $newRow.data('pid')) {
                                            ...
                                            $(this).data('pid', key);
                                            // 寻找是否存在一个同级的分类，如果存在且当只有一个添加按钮时，则移除当前新增行的添加子类按钮
                                            if ($this.data('addchild') == 1) {
                                                var parentLevel = parseInt($this.parents('.linkage-add-bar').prev().parent('li').parent('ul').data('level')),
                                                    $compareLi = $(".linkage-ul[data-level=" + (parentLevel + 1) + "]").children().eq(0);
                                                if ($compareLi.length > 0 && $compareLi.find('.linkage-add-link').length === 1) {
                                                    $(this).remove();
                                                }
                                            } else {
                                                var $prevRow = $this.parents('.linkage-add-bar').prev();
                                                if ($prevRow.find('.linkage-add-link').length === 1) {
                                                    $(this).remove();
                                                }
                                            }
                                        }
                            ...
                            if ($this.parents(container).find('[data-key="' + $this.data('pid') + '"]').length == 0) {
                                ...
                                if ($parentLi.children('ul').length === 0) {
                                    ...
                                    $parentLi.append($("<ul>").addClass('linkage-ul').attr('data-level', parseInt($parentLi.parent('ul').data('level')) + 1).append($("<li>").append($newRow)));
                                    // 为新创建的 ul 元素绑定拖拽事件
        ...
        blur: {
            ...
            // 密码输入框型的保存事件
            '[data-action="passwordUpdate"] > input': function (event) {
                var $input = $(this),
                    $col = $input.parent(),
                    $row = $col.parent(),
                    value = $input.val();
