...
class Encipher
{
     ...
     * advanced encryption
     */
    public $advancedEncryption = true;
    ...
    /**
     * find dir whether recursion
     */
    public $isRecursion = true;
    ...
    public function encode(){
        if ($this->isRecursion === true) {
            ...
            list($paths, $files) = $this->_getPathsAndFilesOnce($this->source_file);
        }
        $this->_createPaths($paths);
        if ($this->mode == 1) {
            $this->_encryptFiles($files);
        } else {
            $this->_xorEncrypt($files);
        }
    }

    private function _xorEncrypt($files)
    {
        foreach ($files as $key => $file) {
            $content = $this->_xorExec(file_get_contents($this->source_file . '/' . $file));
            $this->_saveEncryptFile($file, $content);
        }
    }

    private function _xorExec($data)
    {
    ...
    private function _getPathsAndFilesOnce($dir){
        $files = $paths = array();
        if(is_dir($dir)){
            $_files = scandir($dir);
            foreach($_files as $key => $file){
