...
TEXT;
    ...
    {
        $this->title = '单表提交的最简易演示';
        $this->hint = '主要掌握表单提交的处理逻辑，体会面向对象编程的便利性';

        // 根据id查询获得模型，如果id值为null，则返回空模型
        $model = Test::findModel($id);
        // 如果前台点击提交按钮，则此处会将前台的数据加载到模型中，并进入判断中
        if ($model->load()) {
            // 验证表单并更新或插入到表中，具体验证方式参看：https://www.yiichina.com/doc/guide/2.0/input-validation
            if ($model->save()) {
                // Ajax请求的成功返回
                return success();
            } else {
                // Ajax请求的失败返回，并将错误信息返回到前台
                return error($model);
            }
        }
