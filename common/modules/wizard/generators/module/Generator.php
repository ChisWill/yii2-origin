...
class Generator extends \common\modules\wizard\Generator
{
     ...
     * 更新配置文件
     */
    public function updateConfig($baseNs)
    {
        $app = explode('\\', $this->moduleNamespace)[0];
        $content = file_get_contents(Yii::getAlias("@{$app}/config/main-local.php"));
        preg_match('/\[.*\'modules\'\s*=>\s*(\[.*\]).*\]\s*;/Us', $content, $match);
        if (isset($match[1])) {
            ...
            $config .= str_repeat(' ', 8) . $this->getModuleConfig($baseNs) . PHP_EOL . str_repeat(' ', 4) . ']';
            $content = preg_replace('/(\[.*\'modules\'\s*=>\s*)(\[.*\])(.*\]\s*;)/Us', '$1' . $config . '$3', $content);
            file_put_contents(Yii::getAlias("@{$app}/config/main-local.php"), $content);
        } else {
            $config = PHP_EOL . str_repeat(' ', 4) . '\'modules\' => [' . PHP_EOL . str_repeat(' ', 8) . $this->getModuleConfig($baseNs) . PHP_EOL . str_repeat(' ', 4) . '],';
            $content = preg_replace('/(\[)(.*\]\s*;)/Us', '$1' . $config . '$2', $content);
            file_put_contents(Yii::getAlias("@{$app}/config/main-local.php"), $content);
        }
    }

    private function getModuleConfig($baseNs)
    {
        return "'{$this->moduleName}' => '{$baseNs}\\Module'";
    }
}
