...
class CwController extends \common\components\WebController
{
    ...
    public function actionCreateData()
    {
        ...
        $gen = function ($date = '') use ($config) {
            for ($i = 1; $i <= $config['max']; $i++) {
                $array = str_split(preg_replace('/[a-z]|0/', '', md5($date)));
                $sum = array_sum($array);
                $product = array_product(array_map('array_product', array_chunk($array, ceil(count($array) / 2))));
                $mod = $product % $sum;
                if ($config['percent'] > 0) {
                    $maxWavePoint = $config['base'] * $config['percent'] / 100 / $config['wave'];
                    if ($maxWavePoint < abs($mod)) {
                        $mod = $maxWavePoint * ($mod / abs($mod));
                    }
                }
                $maxWavePoint = $mod * $config['wave'];
            }
        };
        // test($gen());
        $gen();
    ...
    {
        $wx = new \common\classes\WeChat;
        // test($wx->getAccessToken());
        return $this->redirect($wx->getUserInfo());
    }
    ...
    public function actionModel()
    {
        ...
        if (get('_pjax') == '#timeArea') {
            return '<a href="' . url(['cw/model', 'type' => 'time']) . '">' . self::$time . '</a>';
        }
        ...
        if ($model->load()) {
            if ($model->save()) {
                ...
                return $model->getFirstError('name');
            }
        }

        return $this->render('model', compact('model'));
    }
