...
class Setting extends \common\components\Model
{
    ...
    public $uploads = [];
    // 配置
    protected static $settingName = 'common_settings';
    // 内部变量
    ...
    public function updateSetting($id, $field, $value)
    {
        $settings = self::getWebSettings();

        if (!$value) {
            $this->addError($field, $this->label($field) . '是必填的');
            return false;
        }
    ...
    {
        $model = Option::findOne(['option_name' => self::$settingName]);
        $model->option_value = serialize($settings);
