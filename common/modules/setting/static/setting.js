$(function () {
    ...
    var createToDelete = function($obj) {
        $obj.attr('src', $obj.attr('src').replace(/add.png/, 'no.png'))
        return $obj;
    };
    // 叉按钮切换成加号按钮
    var deleteToCreate = function($obj) {
        $obj.attr('src', $obj.attr('src').replace(/no.png/, 'add.png'))
        return $obj;
    }

    // input[type=file]事件绑定
    $("#webSettingContent").on('change', 'input[type="file"]', function() {
    ...
    $("#topParentUl").on('click', '.topMenuList', function () {
        ...
        $.get(window.location.href, {
            ...
            $(".topMenuList.selected").removeClass('selected');
            $this.addClass('selected');
            if (msg.state) {
                $("#webSettingContent").html(msg.info);
                keepShowMode();
            } else {
                $.alert(msg.info);
            }
    ...
    // 二级菜单添加事件绑定
    $("#webSettingContent").on('click', '#addParentLink', function () {
        if ($(this).hasClass('noSubmit')) {
            $("#addParentSpan").prev().remove();
            deleteToCreate($(this)).removeClass('noSubmit');
        } else {
            var newParentInput = '<span>';
    ...
    // 二级菜单创建
    $("#webSettingContent").on('click', '#newParentSubmit', function () {
        ...
        $.post($("#addParentLink").attr('href') + '?nowTopId=' + nowTopId, {
    ...
    // 配置创建按钮
    $("#webSettingContent").on('click', '.addChildLink', function () {
    ...
    // 配置类型下拉事件
    $("#webSettingContent").on('change', '.addChildSelect', function () {
        var value = $(this).val();
        var pid = $(this).attr('pid');
        switch (value) {
            case 'radio':
    ...
    // 配置保存事件
    $("#webSettingContent").on('click', ".newChildSubmit", function () {
        var pid = $(this).attr('pid');
    ...
    // 配置保存按钮
    $("#webSettingContent").on('click', "#settingSubmit", function () {
    ...
    // 配置删除按钮
    $("#webSettingContent").on('click', '.deleteItemLink', function () {
        var $img = $(this),
            extraInfo = '';
        if ($img.data('parent')) {
            extraInfo = '及其下的所有配置项';
