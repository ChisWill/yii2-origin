...
class AuthItem extends \common\components\ARModel
{
    ...
    public static function getRoleQuery()
    {
        return self::find()
            ->with('children')
            ->where(['type' => self::TYPE_ROLE, 'description' => self::getCurrentAppId()])
            ->indexBy('name')
            ->orderBy('name');
    }
    ...
    public static function getFileActionList($filterCtrlList = [])
    {
        $app = self::getCurrentAppId();
        ...
        foreach ($files as $filePath) {
            ...
            if ($matchNums > 0) {
                foreach ($matches as $key => $match) {
                    $result[$app . $controllerName . $match[2]] = $match[1];
                }
            }
        }
    ...
    public static function getAuthItemQuery()
    {
        return self::getPermissionQuery()
            ...
            ->andWhere('name REGEXP BINARY "' . self::getCurrentAppId() . '[A-Z]\w*"')
            ->orderBy('name ASC');
    }
